...
import { Box, Button, SegmentedControl, Text, Slider } from "@mantine/core";
import {
  IconPlayerPlay,
  IconPlayerPause,
  IconRestore,
...
export function ReplayControls() {
  ...
  const visibleCounts = useMemo(() => {
    if (!currentMission || !replayCurrentTime)
      return { dropPoints: 0, waypoints: 0 };
    ...
    <Box>
        ...
        marks={[
          {
            ...
            label: formatTimeOnly(new Date(replayStartTime).toISOString()),
          },
          {
            value: replayEndTime,
            label: formatTimeOnly(new Date(replayEndTime).toISOString()),
          },
        ]}
      />
      ...
      >
        <Text size="xs" c="dimmed">
          {t("replay.current")}:{" "}
          {formatTimeOnly(new Date(currentTimeValue).toISOString())}
        </Text>
        <Text size="xs" c="dimmed">
