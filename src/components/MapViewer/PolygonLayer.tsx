...
export function PolygonLayer() {
  ...
  if (currentMission.missionSettings) {
    currentMission.missionSettings.forEach((settings, index) => {
      ...
      if (selectedSourceFiles.has(filename) && settings.polygon && settings.polygon.length >= 3) {
  ...
  if (wdmPolygons.length === 0 && currentMission.flightLog.polygon && currentMission.flightLog.polygon !== '-1') {
