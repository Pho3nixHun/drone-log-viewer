...
export function MissionParameters() {
  ...
  if (!currentMission?.missionSettings || currentMission.missionSettings.length === 0) return null
  ...
  const renderMissionCard = (settings: any, originalIndex: number) => (
    <Stack key={settings.filename || `wdm-${originalIndex}`} gap="sm" p="sm" style={{ 
      ...
      <Group gap="xs" mb="xs">
        ...
        <Text size="sm" fw={600}>{settings.info.name}</Text>
        <Badge size="xs" variant="light" color="green">
          {settings.filename ? settings.filename.replace('.wdm', '') : `WDM ${originalIndex + 1}`}
        </Badge>
      </Group>
      ...
      <Group gap="sm">
        ...
        <div style={{ flex: 1 }}>
          <Text size="sm" fw={500}>{t('mission.lineSpacing')}</Text>
          <Text size="sm" c="dimmed">{settings.missionParams.dDL}m</Text>
        </div>
        <div style={{ textAlign: 'right' }}>
          <Text size="xs" c="dimmed">{t('mission.fieldArea')}</Text>
      ...
      <Group gap="sm">
        ...
        <div style={{ flex: 1 }}>
          <Text size="sm" fw={500}>{t('mission.altitude')}</Text>
          <Text size="sm" c="dimmed">{settings.missionParams.altitude}m</Text>
        </div>
        <div style={{ textAlign: 'right' }}>
          <Text size="xs" c="dimmed">{t('mission.speed')}</Text>
      ...
      <Group gap="sm">
        ...
        <div style={{ flex: 1 }}>
          <Text size="sm" fw={500}>{t('mission.angle')}</Text>
          <Text size="sm" c="dimmed">{settings.missionParams.angle.toFixed(1)}°</Text>
        </div>
        <div style={{ textAlign: 'right' }}>
          <Text size="xs" c="dimmed">{t('mission.dropDistance')}</Text>
  ...
  return (
    <Card withBorder p="lg" h="100%" style={{ backgroundColor: '#fafbfc' }}>
      <Stack gap="md">
        <div>
          <Text size="lg" fw={600}>Mission Parameters</Text>
          <Text size="sm" c="dimmed">WDM file settings and field configurations</Text>
        </div>
