...
function createDBF(features: ShapefileFeature[]): Uint8Array {
  ...
  features.forEach((feature, index) => {
    ...
    if (feature.geometry.type === "Point") {
      lat = (feature.geometry.coordinates as [number, number])[1]
        .toFixed(6)
        .padStart(8, " ");
      lng = (feature.geometry.coordinates as [number, number])[0]
        .toFixed(6)
        .padStart(8, " ");
    } else {
      // For polygons, use centroid
      lat = (feature.properties.latitude as number).toFixed(6).padStart(8, " ");
      lng = (feature.properties.longitude as number)
        .toFixed(6)
        .padStart(8, " ");
    }
    ...
    // SOURCEIDX
    const sourceIdx = feature.properties.sourceIndex
      .toString()
      .padStart(8, " ");
...
function createSHP(
  ...
  const filteredFeatures = features.filter(
    (f) => f.geometry.type === geometryType,
  );
  ...
  if (geometryType === "Point") {
    allLats = filteredFeatures.map(
      (f) => (f.geometry.coordinates as [number, number])[1],
    );
    allLngs = filteredFeatures.map(
      (f) => (f.geometry.coordinates as [number, number])[0],
    );
  } else {
...
export async function exportToShapefile(
  ...
  if (selectedLayers.has("dropPoints")) {
    dropPoints.forEach((point, index) => {
      if (
        point.latitude !== 0 &&
        point.longitude !== 0 &&
        (!point.sourceFile || selectedSourceFiles.has(point.sourceFile))
      ) {
        features.push({
          type: "Feature",
          geometry: {
  ...
  if (selectedLayers.has("polygon") && mission.missionSettings) {
    ...
    if (selectedLayers.has("missionWaypoints")) {
      ...
      const missionWaypointsCount = mission.missionSettings.reduce(
        (count, settings, index) => {
          const filename = settings.filename || `wdm-${index}`;
          return (
            count +
            (selectedSourceFiles.has(filename) && settings.missionWaypoints
              ? settings.missionWaypoints.length
              : 0)
          );
        },
        0,
    ...
    mission.missionSettings.forEach((settings, settingsIndex) => {
      ...
      if (
        ...
        const coords = settings.polygon.map(
  ...
  if (
    ...
    mission.missionSettings.length > 1
  ) {
    ...
    if (enabledPolygons.length > 1) {
      ...
      const unionCentroid = calculateCentroid(
        unionPolygon.map(([lat, lng]) => [lng, lat] as [number, number]),
      );

      // Add union polygon as a feature
      const unionCoords = unionPolygon.map(
        ([lat, lng]) => [lng, lat] as [number, number],
