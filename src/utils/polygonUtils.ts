...
export function squareDegreesToHectares(
  ...
  const areaSquareMeters =
    areaDegrees * metersPerDegreeLat * metersPerDegreeLng;
...
export function isPointInPolygon(
  ...
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    const xi = polygon[i][1],
      yi = polygon[i][0]; // lng, lat
    const xj = polygon[j][1],
      yj = polygon[j][0]; // lng, lat
...
export function polygonsIntersect(
  ...
  for (let i = 0; i < poly1.length; i++) {
    ...
    const p2 = {
      x: poly1[(i + 1) % poly1.length][1],
      y: poly1[(i + 1) % poly1.length][0],
    };

    for (let j = 0; j < poly2.length; j++) {
      const p3 = { x: poly2[j][1], y: poly2[j][0] };
      const p4 = {
        x: poly2[(j + 1) % poly2.length][1],
        y: poly2[(j + 1) % poly2.length][0],
      };
  ...
  for (let i = 0; i < independentPolygons.length; i++) {
    for (let j = i + 1; j < independentPolygons.length; j++) {
      ...
      for (let e1 = 0; e1 < poly1.length; e1++) {
        ...
        const p2 = {
          x: poly1[(e1 + 1) % poly1.length][1],
          y: poly1[(e1 + 1) % poly1.length][0],
        };

        for (let e2 = 0; e2 < poly2.length; e2++) {
          const p3 = { x: poly2[e2][1], y: poly2[e2][0] };
          const p4 = {
            x: poly2[(e2 + 1) % poly2.length][1],
            y: poly2[(e2 + 1) % poly2.length][0],
          };
    ...
    .sort((a, b) => {
      ...
      if (Math.abs(angleA - angleB) < 1e-10) {
        ...
        const distA = Math.sqrt(
          Math.pow(a.x - bottomPoint.x, 2) + Math.pow(a.y - bottomPoint.y, 2),
        );
        const distB = Math.sqrt(
          Math.pow(b.x - bottomPoint.x, 2) + Math.pow(b.y - bottomPoint.y, 2),
        );
        return distA - distB;
      }
 ...
 */
export function calculateTotalFieldArea(
  ...
  if (validPolygons.length === 1) {
    ...
    const avgLat =
      validPolygons[0].reduce((sum, coord) => sum + coord[0], 0) /
      validPolygons[0].length;
