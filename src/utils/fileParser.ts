  ...
  for (let i = 0; i < result.length; i++) {
    if (isInvalidTimestamp(result[i].date)) {
      ...
      if (prevValidIndex !== -1 && nextValidIndex !== -1) {
        ...
        interpolatedTime =
          prevTime + (nextTime - prevTime) * interpolationFactor;
...
export async function parseMultipleJSONFiles(
  ...
  if (files.length === 1) {
    ...
    const { points: fixedDropPoints, fixedCount: dropPointsFixed } =
      interpolateTimestamps(singleMission.flightLog.dropPoints);
    const { points: fixedWaypoints, fixedCount: waypointsFixed } =
      interpolateTimestamps(singleMission.flightLog.waypoints);
  ...
  for (const file of files) {
    try {
      ...
      const { points: fixedDropPoints, fixedCount: dropPointsFixed } =
        interpolateTimestamps(mission.flightLog.dropPoints);
      const { points: fixedWaypoints, fixedCount: waypointsFixed } =
        interpolateTimestamps(mission.flightLog.waypoints);
  ...
  const startDate =
    allTimestamps.length > 0
      ...
      : baseMission.flightLog.startDate;
  const endDate =
    allTimestamps.length > 0
      ? new Date(allTimestamps[allTimestamps.length - 1]).toISOString()
      : baseMission.flightLog.endDate;
...
export function calculateMissionStats(mission: MissionLog): MissionStats {
  ...
  const { averageDropDistance, averageDropLineDistance } =
    calculateDropDistances(dropPoints);
...
function calculateDropDistances(dropPoints: unknown[]): {
  ...
  const validPoints = dropPoints.filter(
    (p): p is { latitude: number; longitude: number; date: string } => {
      return (
        p !== null &&
        typeof p === "object" &&
        "latitude" in p &&
  ...
  const averageDropDistance =
    dropDistances.length > 0
      ? dropDistances.reduce((sum, dist) => sum + dist, 0) /
        dropDistances.length
      : 0;
  ...
  let currentLine: { latitude: number; longitude: number; date: string }[] = [
    sortedPoints[0],
  ];
  ...
  for (let i = 1; i < sortedPoints.length; i++) {
    ...
    // Calculate time gap
    const timeGap =
      (new Date(currPoint.date).getTime() -
  ...
  const averageDropLineDistance =
    allDropLineDistances.length > 0
      ? allDropLineDistances.reduce((sum, dist) => sum + dist, 0) /
        allDropLineDistances.length
      : 0;
